@interface IamBizNativeToCppManager : NSObject {
    ...
    struct shared_ptr<biz::IamBizNativeToCppManagerCppCover> { struct IamBizNativeToCppManagerCppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<biz::IamBizNativeToCppManagerDispatcher> { struct IamBizNativeToCppManagerDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<biz::IamBizNativeToCppManagerDispatcher> { struct IamBizNativeToCppManagerDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)sendMessageAsync:(id)a0 completion:(id /* block */)a1;
- (void)sendMessageAsync:(id)a0 on:(id)a1 completion:(id /* block */)a2;
- (id)syncSendMessage:(id)a0;
- (id)getOfficialAccountSummaryInfo:(id)a0;
- (id)getNotifyPair;
- (void)subscribeNotifyMessageEvent:(id)a0 on:(id)a1 eventBlock:(id /* block */)a2;
- (void)subscribeNotifyMessageEvent:(id)a0 eventBlock:(id /* block */)a1;
- (void)unsubscribeNotifyMessageEvent:(id)a0;
- (void)onEnterAsync:(id /* block */)a0;
- (void)onEnterAsync:(id)a0 completion:(id /* block */)a1;
- (void)initIamEnvironment:(id)a0;
- (void)dealloc;
- (void).cxx_destruct;
- (id).cxx_construct;
