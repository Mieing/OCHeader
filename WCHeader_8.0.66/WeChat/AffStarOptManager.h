@interface AffStarOptManager : NSObject {
    ...
    struct shared_ptr<star::AffStarOptManagerCppCover> { struct AffStarOptManagerCppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<star::AffStarOptManagerDispatcher> { struct AffStarOptManagerDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<star::AffStarOptManagerDispatcher> { struct AffStarOptManagerDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)starOperateAsync:(id)a0 completion:(id /* block */)a1;
- (void)starOperateAsync:(id)a0 on:(id)a1 completion:(id /* block */)a2;
- (void)addStarAsync:(id)a0 completion:(id /* block */)a1;
- (void)addStarAsync:(id)a0 on:(id)a1 completion:(id /* block */)a2;
- (void)downloadStarSnapshotAsync:(id)a0 savePath:(id)a1 completion:(id /* block */)a2;
- (void)downloadStarSnapshotAsync:(id)a0 savePath:(id)a1 on:(id)a2 completion:(id /* block */)a3;
- (void)dealloc;
- (void).cxx_destruct;
- (id).cxx_construct;
