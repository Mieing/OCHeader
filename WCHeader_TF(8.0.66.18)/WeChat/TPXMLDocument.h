@interface TPXMLDocument : NSObject {
    struct _xmlDoc { void *x0; int x1; char *x2; struct _xmlNode *x3; struct _xmlNode *x4; struct _xmlNode *x5; struct _xmlNode *x6; struct _xmlNode *x7; struct _xmlDoc *x8; int x9; int x10; struct _xmlDtd *x11; struct _xmlDtd *x12; struct _xmlNs *x13; char *x14; char *x15; void *x16; void *x17; char *x18; int x19; struct _xmlDict *x20; void *x21; int x22; int x23; } *xmlDoc_;
}

- (id)initWithXMLString:(id)a0 options:(unsigned int)a1 error:(id *)a2;
- (id)initWithData:(id)a0 options:(unsigned int)a1 error:(id *)a2;
- (id)initWithRootElement:(id)a0;
- (void)addStringsCacheToDoc;
- (id)description;
- (void)dealloc;
- (id)rootElement;
- (id)XMLData;
- (void)setVersion:(id)a0;
- (void)setCharacterEncoding:(id)a0;
- (id)nodesForXPath:(id)a0 error:(id *)a1;
- (id)nodesForXPath:(id)a0 namespaces:(id)a1 error:(id *)a2;

@end
