@interface IamBizProfileNativeToCppManager : NSObject {
    ...
    struct shared_ptr<biz::IamBizProfileNativeToCppManagerCppCover> { struct IamBizProfileNativeToCppManagerCppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<biz::IamBizProfileNativeToCppManagerDispatcher> { struct IamBizProfileNativeToCppManagerDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<biz::IamBizProfileNativeToCppManagerDispatcher> { struct IamBizProfileNativeToCppManagerDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)setProfileOutboxAppmsgIds:(id)a0;
- (void)getProfileOutboxAppmsgIdsAsync:(id /* block */)a0;
- (void)getProfileOutboxAppmsgIdsAsync:(id)a0 completion:(id /* block */)a1;
- (void)deleteProfileOutboxAppmsgId:(int)a0;
- (void)getCacheOutboxMessagesAsync:(id /* block */)a0;
- (void)getCacheOutboxMessagesAsync:(id)a0 completion:(id /* block */)a1;
- (void)fetchOutboxMessagesAsync:(id /* block */)a0;
- (void)fetchOutboxMessagesAsync:(id)a0 completion:(id /* block */)a1;
- (void)deleteAppmsgAsync:(int)a0 index:(int)a1 completion:(id /* block */)a2;
- (void)deleteAppmsgAsync:(int)a0 index:(int)a1 on:(id)a2 completion:(id /* block */)a3;
- (void)getTempUrlAsync:(int)a0 index:(int)a1 completion:(id /* block */)a2;
- (void)getTempUrlAsync:(int)a0 index:(int)a1 on:(id)a2 completion:(id /* block */)a3;
- (void)startPollingOutboxMessages;
- (void)stopPollingOutboxMessages;
- (void)subscribeOutboxMessagesUpdatedEvent:(id)a0 on:(id)a1 eventBlock:(id /* block */)a2;
- (void)subscribeOutboxMessagesUpdatedEvent:(id)a0 eventBlock:(id /* block */)a1;
- (void)unsubscribeOutboxMessagesUpdatedEvent:(id)a0;
- (void)dealloc;
- (void).cxx_destruct;
- (id).cxx_construct;
