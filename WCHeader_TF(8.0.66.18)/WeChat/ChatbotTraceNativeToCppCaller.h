@interface ChatbotTraceNativeToCppCaller : NSObject {
    ...
    struct shared_ptr<chatbot::ChatbotTraceNativeToCppCallerCppCover> { struct ChatbotTraceNativeToCppCallerCppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<chatbot::ChatbotTraceNativeToCppCallerDispatcher> { struct ChatbotTraceNativeToCppCallerDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<chatbot::ChatbotTraceNativeToCppCallerDispatcher> { struct ChatbotTraceNativeToCppCallerDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)serviceInit;
- (void)serviceClearData;
- (void)fetchChatBotTraceItemAsync:(id /* block */)a0;
- (void)fetchChatBotTraceItemAsync:(id)a0 completion:(id /* block */)a1;
- (void)insertChatBotTraceItemAsync:(id)a0 completion:(id /* block */)a1;
- (void)insertChatBotTraceItemAsync:(id)a0 on:(id)a1 completion:(id /* block */)a2;
- (void)getAllTraceItemAsync:(int)a0 completion:(id /* block */)a1;
- (void)getAllTraceItemAsync:(int)a0 on:(id)a1 completion:(id /* block */)a2;
- (id)getTraceItemDataPath:(id)a0 dataFmt:(id)a1 isThumb:(BOOL)a2;
- (void)dealloc;
- (void).cxx_destruct;
- (id).cxx_construct;
