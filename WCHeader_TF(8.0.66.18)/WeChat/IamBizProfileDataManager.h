@interface IamBizProfileDataManager : NSObject {
    ...
    struct shared_ptr<biz::IamBizProfileDataManagerCppCover> { struct IamBizProfileDataManagerCppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<biz::IamBizProfileDataManagerDispatcher> { struct IamBizProfileDataManagerDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<biz::IamBizProfileDataManagerDispatcher> { struct IamBizProfileDataManagerDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)getCacheProfileDataAsync:(id)a0 tabType:(int)a1 inBrandManagerMode:(BOOL)a2 completion:(id /* block */)a3;
- (void)getCacheProfileDataAsync:(id)a0 tabType:(int)a1 inBrandManagerMode:(BOOL)a2 on:(id)a3 completion:(id /* block */)a4;
- (void)fetchProfileDataAsync:(id)a0 tabType:(int)a1 inBrandManagerMode:(BOOL)a2 offset:(const void *)a3 completion:(id /* block */)a4;
- (void)fetchProfileDataAsync:(id)a0 tabType:(int)a1 inBrandManagerMode:(BOOL)a2 offset:(const void *)a3 on:(id)a4 completion:(id /* block */)a5;
- (void)updateListenLaterAsync:(id)a0 isAdd:(BOOL)a1 completion:(id /* block */)a2;
- (void)updateListenLaterAsync:(id)a0 isAdd:(BOOL)a1 on:(id)a2 completion:(id /* block */)a3;
- (void)dealloc;
- (void).cxx_destruct;
- (id).cxx_construct;
