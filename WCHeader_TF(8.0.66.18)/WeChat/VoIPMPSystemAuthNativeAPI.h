@interface VoIPMPSystemAuthNativeAPI : NSObject {
    ...
    struct shared_ptr<voipmp::VoIPMPSystemAuthNativeAPICppCover> { struct VoIPMPSystemAuthNativeAPICppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<voipmp::VoIPMPSystemAuthNativeAPIDispatcher> { struct VoIPMPSystemAuthNativeAPIDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<voipmp::VoIPMPSystemAuthNativeAPIDispatcher> { struct VoIPMPSystemAuthNativeAPIDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)requestAsync:(int)a0 completion:(id /* block */)a1;
- (void)requestAsync:(int)a0 on:(id)a1 completion:(id /* block */)a2;
- (void)dealloc;
- (void).cxx_destruct;
- (id).cxx_construct;
