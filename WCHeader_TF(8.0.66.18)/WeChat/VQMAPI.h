@interface VQMAPI : NSObject {
    ...
    struct shared_ptr<vqm::VQMAPICppCover> { struct VQMAPICppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<vqm::VQMAPIDispatcher> { struct VQMAPIDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<vqm::VQMAPIDispatcher> { struct VQMAPIDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)launch;
- (void)attachObserver:(id)a0;
- (void)detachObserver:(id)a0;
- (void)begin;
- (void)end;
- (BOOL)isActived;
- (void)updateProfile:(int)a0 bytesInfo:(id)a1;
- (void)updateProfileSync:(int)a0 bytesInfo:(id)a1;
- (void)loadProfileAsync:(int)a0 completion:(id /* block */)a1;
- (void)loadProfileAsync:(int)a0 on:(id)a1 completion:(id /* block */)a2;
- (void)confirm;
- (unsigned long long)timestampMSCurrentSteady;
- (unsigned long long)timestampMSCurrentSystem;
- (void)updateEvent:(int)a0 bytesInfo:(id)a1;
- (void)dealloc;
- (void).cxx_destruct;
- (id).cxx_construct;
