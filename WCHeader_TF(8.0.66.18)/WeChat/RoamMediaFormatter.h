@interface RoamMediaFormatter : NSObject {
    ...
    struct shared_ptr<affroam::RoamMediaFormatterCppCover> { struct RoamMediaFormatterCppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<affroam::RoamMediaFormatterDispatcher> { struct RoamMediaFormatterDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<affroam::RoamMediaFormatterDispatcher> { struct RoamMediaFormatterDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (id)getHDLiveMediaID:(long long)a0;
- (id)getLiveMediaID:(long long)a0;
- (id)getHDMediaID:(long long)a0;
- (id)getMediaID:(long long)a0;
- (id)getThumbMediaID:(long long)a0;
- (id)getGNThumbMediaID:(long long)a0 index:(int)a1;
- (id)getGNDataMediaID:(long long)a0 index:(int)a1;
- (id)getFavThumbMediaID:(long long)a0 suffix:(id)a1;
- (id)getFavMediaID:(long long)a0 suffix:(id)a1;
- (id)getHistoryThumbMediaID:(long long)a0 suffix:(id)a1;
- (id)getHistoryMediaID:(long long)a0 suffix:(id)a1;
- (void)dealloc;
- (void).cxx_destruct;
- (id).cxx_construct;
