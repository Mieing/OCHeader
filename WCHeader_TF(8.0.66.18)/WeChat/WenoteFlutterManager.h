@interface WenoteFlutterManager : NSObject {
    ...
    struct shared_ptr<wenote::WenoteFlutterManagerCppCover> { struct WenoteFlutterManagerCppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<wenote::WenoteFlutterManagerDispatcher> { struct WenoteFlutterManagerDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<wenote::WenoteFlutterManagerDispatcher> { struct WenoteFlutterManagerDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)createWeNoteFlutterInstanceAsync:(id)a0 delegate:(id)a1 completion:(id /* block */)a2;
- (void)createWeNoteFlutterInstanceAsync:(id)a0 delegate:(id)a1 on:(id)a2 completion:(id /* block */)a3;
- (void)getWeNoteFlutterInstanceAsync:(id)a0 completion:(id /* block */)a1;
- (void)getWeNoteFlutterInstanceAsync:(id)a0 on:(id)a1 completion:(id /* block */)a2;
- (void)destoryWeNoteFlutterInstanceAsync:(id)a0 completion:(id /* block */)a1;
- (void)destoryWeNoteFlutterInstanceAsync:(id)a0 on:(id)a1 completion:(id /* block */)a2;
- (void)dealloc;
- (void).cxx_destruct;
- (id).cxx_construct;
