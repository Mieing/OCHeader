@interface RoamTaskManager : NSObject {
    ...
    struct shared_ptr<affroam::RoamTaskManagerCppCover> { struct RoamTaskManagerCppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<affroam::RoamTaskManagerDispatcher> { struct RoamTaskManagerDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<affroam::RoamTaskManagerDispatcher> { struct RoamTaskManagerDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)startBackupAsync:(unsigned long long)a0 isAutoBackup:(BOOL)a1 completion:(id /* block */)a2;
- (void)startBackupAsync:(unsigned long long)a0 isAutoBackup:(BOOL)a1 on:(id)a2 completion:(id /* block */)a3;
- (void)startRestoreAsync:(unsigned long long)a0 conversationIdList:(id)a1 completion:(id /* block */)a2;
- (void)startRestoreAsync:(unsigned long long)a0 conversationIdList:(id)a1 on:(id)a2 completion:(id /* block */)a3;
- (void)stopAsync:(unsigned long long)a0 completion:(id /* block */)a1;
- (void)stopAsync:(unsigned long long)a0 on:(id)a1 completion:(id /* block */)a2;
- (id)getTaskInfo:(unsigned long long)a0;
- (void)subscribeProgressAndTransferStatsChangedEvent:(id)a0 on:(id)a1 eventBlock:(id /* block */)a2;
- (void)subscribeProgressAndTransferStatsChangedEvent:(id)a0 eventBlock:(id /* block */)a1;
- (void)unsubscribeProgressAndTransferStatsChangedEvent:(id)a0;
