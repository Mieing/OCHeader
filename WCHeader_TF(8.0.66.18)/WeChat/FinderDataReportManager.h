@interface FinderDataReportManager : NSObject {
    ...
    struct shared_ptr<global_finder_report::FinderDataReportManagerCppCover> { struct FinderDataReportManagerCppCover *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_cover;
    struct weak_ptr<global_finder_report::FinderDataReportManagerDispatcher> { struct FinderDataReportManagerDispatcher *__ptr_; struct __shared_weak_count *__cntrl_; } cpp_dispatcher;
}

+ (id)shareInstance;
+ (id)buildZidlObjForHolder:(const void *)a0 svrIdentity:(const void *)a1 refCntManager:(void *)a2;

- (struct shared_ptr<global_finder_report::FinderDataReportManagerDispatcher> { struct FinderDataReportManagerDispatcher *x0; struct __shared_weak_count *x1; })getDispatcher;
- (void)setDispatcher:(const void *)a0;
- (id)initForZidlPrivate;
- (id)zidlObjToHolder;
- (void)pushStack:(id)a0;
- (void)popStack:(id)a0;
- (void)changeStack:(id)a0 toPageRecord:(id)a1 isCreatNewStack:(BOOL)a2;
- (void)onPageAppear:(id)a0;
- (void)onPageDisappear:(id)a0;
- (void)onAppIn:(id)a0;
- (void)onAppOut:(id)a0;
- (void)dealloc;
- (void).cxx_destruct;
