@class LynxContext, NSString, LynxThreadSafeDictionary, NSLock;
@protocol LynxCanvasManager;

@interface LynxCanvasHelper : NSObject <LynxKryptonHelper, LynxCanvasManager> {
    LynxContext *_weakLynxContext;
    LynxThreadSafeDictionary *_serviceMap;
    id<LynxCanvasManager> _canvas_manager;
    struct weak_ptr<lynx::canvas::CanvasRuntimeMediator> { struct CanvasRuntimeMediator *__ptr_; struct __shared_weak_count *__cntrl_; } _canvas_runtime_mediator_weak;
    NSString *_temporaryDirectory;
    NSLock *_temporaryDirectoryLock;
}
