...
@interface IESLiveLinkRTCRemoteRenderNode : IESLiveLinkBaseNode <IESLiveLinkInputNode> {
    struct _opaque_pthread_mutex_t { long long __sig; char __opaque[56]; } _mutexLock;
}

@property (retain, nonatomic) NSMapTable *renderSessions;
@property (retain, nonatomic) NSMutableDictionary *videoFrames;
...
- (void)addRenderSession:(id)a0 userID:(id)a1;
- (void)removeAllRenderSession;
- (id)renderSessionsUids;
- (void)removeRenderSession:(id)a0;
- (void).cxx_destruct;
