...
+ (double)reviseProcessStartTime:(double)a0;
+ (BOOL)isSystemBackgroundLaunch;
+ (void)trackFirstFrameAndTrigerEventWithModel:(id)a0 blockedWithAlertView:(BOOL)a1;
+ (void)updateIsFirstPlay:(BOOL)a0;
+ (void)updateFeedHotPageAppeared:(BOOL)a0;
+ (void)updateFeedHotPageDisappeared:(BOOL)a0;
+ (BOOL)p_isFromFamiliarCache:(id)a0;
+ (id)dataCacheManager;
+ (void)awe_feedReadyBootTasksDelay;
+ (id)splashCacheManager;
+ (BOOL)hadValidDeatilsWithFirstFrame;
+ (id)p_isFromFeedCacheNumber:(id)a0;
+ (long long)firstFrameFeedType;
+ (id)p_isFromFamiliarCacheNumber:(id)a0;
+ (void)addMetricInfoWithName:(id)a0 start:(double)a1 endTime:(double)a2 metrics:(id)a3;
+ (void)clearDetailsWithFirstFrame;
+ (id)firstFrameLoadTimeWithModel:(id)a0 blockedWithAlertView:(BOOL)a1;
