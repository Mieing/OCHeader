@class UIFont, NSString, NSAttributedString, UITextRange, IESIMTextViewInternal, NSObject, NSParagraphStyle, UIColor;
@protocol IESIMGrowingTextViewDelegate;

@interface IESIMGrowingTextView : UIView <IESIMGrowingTextViewProtcol, UITextViewDelegate, UITextDragDelegate> {
    int maxNumberOfLines;
    int minNumberOfLines;
    long long keyboardType;
    struct UIEdgeInsets { double top; double left; double bottom; double right; } contentInset;
}

@property (nonatomic) BOOL enableOptMaxAndMinLineAlgorithm;
@property (nonatomic) BOOL enableScrollEnableOpt;
@property (nonatomic) BOOL didFlashScrollIndicators;
@property int maxNumberOfLines;
@property int minNumberOfLines;
@property (nonatomic) int maxHeight;
@property (nonatomic) int minHeight;
@property BOOL animateHeightChange;
@property BOOL shouldCheckHeightChange;
@property double animationDuration;
@property (retain, nonatomic) NSString *placeholder;
@property (retain, nonatomic) UIColor *placeholderColor;
@property (retain, nonatomic) NSParagraphStyle *placeholderParagraphStyle;
@property (retain, nonatomic) NSParagraphStyle *contentParagraphStyle;
@property (retain, nonatomic) IESIMTextViewInternal *internalTextView;
@property (weak, nonatomic) NSObject<IESIMGrowingTextViewDelegate> *delegate;
@property (retain, nonatomic) NSString *text;
@property (retain, nonatomic) UIFont *font;
@property (retain, nonatomic) UIColor *textColor;
@property (nonatomic) long long textAlignment;
@property (nonatomic) struct _NSRange { unsigned long long location; unsigned long long length; } selectedRange;
@property (nonatomic, getter=isEditable) BOOL editable;
@property (nonatomic) unsigned long long dataDetectorTypes;
@property (nonatomic) long long returnKeyType;
@property (nonatomic) long long keyboardType;
@property struct UIEdgeInsets { double x0; double x1; double x2; double x3; } contentInset;
@property (nonatomic) struct UIEdgeInsets { double x0; double x1; double x2; double x3; } textContainerInset;
@property (nonatomic) BOOL disableCaretFixForIOS7;
@property (nonatomic) double lineFragmentPadding;
@property (nonatomic) long long lineBreakMode;
@property (nonatomic) double lineSpacing;
@property (nonatomic) long long maximumNumberOfLines;
@property (nonatomic) BOOL isScrollable;
@property (nonatomic) BOOL disableGrowing;
@property (nonatomic) BOOL disableFixOverscroll;
@property (nonatomic) BOOL enablesReturnKeyAutomatically;
@property (copy, nonatomic) NSAttributedString *attributedText;
@property (readonly, nonatomic) UITextRange *markedTextRange;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (void)refreshHeight;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 config:(id)a1;
- (void)growingTextViewDidChange;
- (void)commonInitialiser;
- (void)commonInitialiser:(id)a0;
- (double)measureHeight;
- (void)resizeTextView:(long long)a0;
- (void)updatePlaceholderHidden;
- (void)resetScrollPositionForIOS7;
- (void)setContentInsetPurely:(struct UIEdgeInsets { double x0; double x1; double x2; double x3; })a0;
- (void)growDidStop;
- (void)setTextViewPastedAction:(id /* block */)a0;
- (void)p_setMaxNumberOfLines:(int)a0;
- (void)p_updateMinOrMaxHeight:(int *)a0 minOrMaxNumberOfLines:(int *)a1 targetNumberOfLines:(int)a2;
- (void)p_setMinNumberOfLines:(int)a0;
- (void)setPlaceholderColor:(id)a0;
- (id)placeholderColor;
- (void).cxx_destruct;
- (BOOL)isFirstResponder;
- (BOOL)resignFirstResponder;
- (void)setBackgroundColor:(id)a0;
- (BOOL)becomeFirstResponder;
- (id)placeholder;
- (id)backgroundColor;
- (void)setPlaceholder:(id)a0;
- (BOOL)hasText;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0;
- (id)initWithCoder:(id)a0;
- (BOOL)textView:(id)a0 shouldInteractWithTextAttachment:(id)a1 inRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a2 interaction:(long long)a3;
- (struct CGSize { double x0; double x1; })sizeThatFits:(struct CGSize { double x0; double x1; })a0;
- (void)touchesEnded:(id)a0 withEvent:(id)a1;
- (void)layoutSubviews;
- (void)scrollRangeToVisible:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a0;
- (id)textDraggableView:(id)a0 itemsForDrag:(id)a1;
- (void)textViewDidBeginEditing:(id)a0;
- (BOOL)textViewShouldBeginEditing:(id)a0;
- (BOOL)textViewShouldEndEditing:(id)a0;
- (void)textViewDidEndEditing:(id)a0;
- (void)textViewDidChange:(id)a0;
- (void)textViewDidChangeSelection:(id)a0;
- (BOOL)textView:(id)a0 shouldChangeTextInRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a1 replacementText:(id)a2;

@end
