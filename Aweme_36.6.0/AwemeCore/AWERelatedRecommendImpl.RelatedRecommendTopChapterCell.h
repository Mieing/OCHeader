@interface AWERelatedRecommendImpl.RelatedRecommendTopChapterCell : UICollectionViewCell {
    void /* unknown type, empty encoding */ chapterInfo;
    void /* unknown type, empty encoding */ index;
    void /* unknown type, empty encoding */ $__lazy_storage_$_container;
    void /* unknown type, empty encoding */ $__lazy_storage_$_playingView;
    void /* unknown type, empty encoding */ $__lazy_storage_$_indexLabel;
    void /* unknown type, empty encoding */ $__lazy_storage_$_contentLabel;
}

- (void).cxx_destruct;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0;
- (id)initWithCoder:(id)a0;

@end
